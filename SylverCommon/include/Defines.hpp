...
namespace Sylver{
    ...
    sizet operator ""_sizet(unsigned long long int);
    uptr operator ""_uptr(unsigned long long int);
    iptr operator ""_iptr(unsigned long long int);

#if defined(SYLVER_PLATFORM_WINDOWS)

#if defined(SYLVER_DLL_EXPORT)
